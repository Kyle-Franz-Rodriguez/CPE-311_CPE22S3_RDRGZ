...
def cross_river(land1, boat = [], land2 = [], counter = 0, capacity = 100):
    ...
    while land1:
        ...
        if len(boat) == 1:
            # If no person left in land1, most likely all items are too heavy to carry
            if all(i.sentient == False for i in land1):
                heads -= len(land1)
            else:
        ...
        if len(land2) == heads:
            ...
            if notcrossed or land1:
                print("Except for",[i.name for i in land1], [i.name for i in notcrossed],"...sadly.\n")
            break
        # Reset counter
        counter = 0
